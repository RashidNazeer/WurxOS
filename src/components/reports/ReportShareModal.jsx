...
export default function ReportShareModal({ report, onClose }) {
  ...
  async function load() {
    ...
    try {
      const list = await listReportShares(report.id);
      setShares(list);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const row = await createReportShare(report.id, { expiresAt });
  ...
  async function handleRevoke(token) {
    if (!confirm('Revoke this link? Anyone with it will no longer be able to open the report.')) return;
    try { await revokeReportShare(token); await load(); }
    catch (e) { setErr(e.message); }
  }
    ...
    <div className="wx-modal-backdrop" onClick={onClose}>
      <div className="wx-modal" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
        <div className="wx-modal-header">
          <div className="wx-modal-title">Share report</div>
        ...
        <div className="wx-modal-body">
          ...
          <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 14 }}>
            Anyone with the link can view this approved report — no login required. Revoke any time.
          </div>
