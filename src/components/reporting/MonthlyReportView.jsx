...
// Column headers for the Top Creators table — widths mirror
...
function CreatorHeader() {
  ...
  return (
    <div className="d-flex align-items-center gap-3 px-3 py-2" style={{ borderTop: `1px solid ${C.line}` }}>
      ...
      <div style={{ width: 140, ...cell }}>GMV Share</div>
...
// Hierarchical donut for GMV breakdown. Donut shows only the
// top-level parents (Affiliate, Organic) so the visualised total
// equals what the user entered as Total GMV — no double-counting.
// Each legend row has a chevron; clicking expands a child list
// showing the parent's sub-components indented underneath.
function HierarchicalGmvDonut({ parents, total, currencySym = '$', size = 180, thickness = 28 }) {
              ...
              >
                {hasChildren ? (
                  <i
                    ...
                    style={{
                      fontSize: '0.6rem', color: C.muted, width: 10, textAlign: 'center',
                ...
                <span style={{ fontSize: '0.78rem', color: C.ink, flex: 1, fontWeight: 600 }}>{p.label}</span>
                <span style={{ fontSize: '0.78rem', fontWeight: 700, color: C.ink, fontVariantNumeric: 'tabular-nums' }}>
                  {currencySym}{v.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                </span>
                <span style={{ fontSize: '0.7rem', color: C.muted, minWidth: 44, textAlign: 'right' }}>
                  ...
                  {visibleChildren.map((c, ci) => {
                    ...
                    return (
                      <div key={ci} className="d-flex align-items-center gap-2 mb-1" style={{ paddingLeft: 4 }}>
                        ...
                        <span style={{ fontSize: '0.72rem', color: C.inkDim, flex: 1 }}>{c.label}</span>
                        <span style={{ fontSize: '0.72rem', fontWeight: 600, color: C.inkDim, fontVariantNumeric: 'tabular-nums' }}>
                          {currencySym}{cv.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                        </span>
                        <span style={{ fontSize: '0.66rem', color: C.muted, minWidth: 40, textAlign: 'right' }}>
                          {childPct.toFixed(1)}%
                        </span>
                      </div>
                    );
...
export default function MonthlyReportView({ report, previousReport, allReports, clientView = false, onActions, reportType = 'monthly' }) {
  ...
  // supplied, this view skips rendering its own action bar. Monthly
  // reports have no Word export, so onExportWord is omitted.
  React.useEffect(() => {
    ...
    onActions({
      ...
      pdfBusy,
      canCopyInsights: userRole === 'tl' || userRole === 'pctl',
      onCopyInsights: handleCopyInsights,
      copyDone: copyState === 'done',
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onActions, clientView, highlighterActive, pdfBusy, copyState, userRole]);
    ...
    <div>
      ...
      <div ref={printRef} className="report-canvas" style={{ background: C.surfaceAlt, padding: '24px', borderRadius: 18, border: `1px solid ${C.line}` }}>
        ...
        {sectEnabled.gmvBreakdown && (() => {
          ...
          // Only Affiliate and Organic are top-level — Live and Video
          // sum INTO Affiliate, Product Card sums INTO Organic. Summing
          // all five double-counts and produced a "Total GMV" in the
          // donut that exceeded the report's top-card GMV. Reported
          // 2026-06-03 (FlyWell: top card $859.95, donut total $1,720).
          const parents = [
            {
              key: 'affiliate',
              label: 'Affiliate GMV',
              value: num(g.affiliateGmv),
              color: '#0891b2',
              children: [
          ...
          return (
            <ContentSection icon="bi-pie-chart-fill" color="#0891b2" title="GMV breakdown" eyebrow="Revenue mix by traffic source">
              <HierarchicalGmvDonut parents={parents} total={total} currencySym={sym} />
            </ContentSection>
          );
        })()}
