-- ============================================================
-- WurxOS v2 — Migration 051: fix brand_switch_apc column name
--
-- Migration 049 ordered brand_assignments by `created_at`, but that
-- column doesn't exist — the table uses `assigned_at` (see migration
-- 004). The error surfaced the moment the Switch APC modal tried to
-- pick the "old APC" from an assigned brand. This migration reissues
-- the function with the correct column name; everything else from
-- 049 (swap semantics, audit rows, opt-in notifications) is kept.
-- ============================================================
...
begin
  ...
  -- Old primary APC on the clicked brand — use assigned_at, the
  -- actual column on brand_assignments (migration 004).
