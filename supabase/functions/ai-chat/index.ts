...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const groundingRules = [
      ...
      '- The KNOWLEDGE comes in TWO kinds and you must NOT confuse them: (1) the WurxOS how-to guides + company SOP library (about OUR app and internal process), and (2) the TIKTOK SHOP KNOWLEDGE block (official TikTok Shop Academy — about TikTok Shop itself). Ground every answer in the relevant one. NEVER call TikTok Shop Academy content "the SOP" or "our SOP" — if a fact comes from the TikTok block, say it is from TikTok Shop / the TikTok Shop Academy. Prefer quoting the actual steps from whichever source you used.',
      ...
      '- For TikTok Shop questions (GMV Max / ads, affiliate & creators, product policy, listings, promotions, LIVE, seller setup), answer from the TIKTOK SHOP KNOWLEDGE section (official TikTok Shop Academy). ALWAYS end an answer that used it with a markdown link to the article, using its Source URL, e.g. [TikTok Shop Academy](<source url>). This citation is REQUIRED whenever you used any TikTok fact — do not omit it.',
      '- Give the concrete guidance the TikTok knowledge contains rather than over-hedging. If the Academy states a specific number or recommendation (e.g. a $150/day budget guideline, a review window, a commission rule), state it plainly as TikTok\'s guidance. Only say "not specified" when the knowledge genuinely lacks it — do not downplay a real recommendation as if it were missing.',
      '- If both TikTok knowledge and the brand\'s live WurxOS data are relevant, blend them (the TikTok best practice AND what the brand\'s actual campaign/numbers show), and point to the right WurxOS page for the live view.',
